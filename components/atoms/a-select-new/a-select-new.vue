<template>
  <div ref="target" class="a-select" :class="classes">
    <div class="a-select__box">
      <div class="a-select__row">
        <div tabindex="0" class="a-select__input" @click="isOpen = !isOpen">
        ...
        <div class="a-select__img" :class="{ 'a-select__img--open': isOpen }">
        ...
        <transition name="a-select__list">
          <div v-if="isOpen && options.length > 0" class="a-select__list">
            <div class="a-select__wrapper" :class="{ multiple: multiple }" @scroll="onScroll">
              <div v-for="(item, index) in options" :key="index">
                <div v-if="!multiple" class="a-select__option" @click="select(item)">
    ...
    <div v-if="showSelectButton" class="a-select__block">
      ...
      <a-button label="Отменить" color="violet-5" size="medium" class="a-select__button" @click="clearOptions" />
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  activeSelect: { type: Object, default: () => null },
  isCanInput: { type: Boolean, default: true },
});

const target = ref(null);
const emit = defineEmits(['save', 'filter', 'select', 'input-search', 'scroll-end']);
const isOpen = ref(false);
const searchLabel = ref('');

let isChangedInput = ref(false);

const computedLabel = computed({
  get() {
    if (!isChangedInput.value) {
...
const handleInput = (e) => {
  isOpen.value = true;
  emit('input-search', e.target.value);
};

const select = (item) => {
  searchLabel.value = item.attributes.name;
  emit('select', item);
  isOpen.value = false;
};
