<template>
  <nuxt-link to="/blog/articles/_id" class="m-articles-card">
    <div class="m-articles-card__img">
      <img :src="getImageUrl(articles.media)" :alt="articles?.name" loading="lazy" />
    </div>

    <div class="m-articles-card__info">
      <h5 class="m-articles-card__title title-h5">
        {{ articles?.name }}
      </h5>
      <span class="m-articles-card__date gray f-text-xxs">{{ articles.formattedDate }}</span>

      <a-button :label="countLabel" :img="arrow" :center="false" color="white" class="m-articles-card__button" />
    </div>
  </nuxt-link>
</template>
...
function getImageUrl(mediaArray) {
  if (mediaArray && mediaArray.length > 0) {
    return mediaArray[0].url;
  }
  return '/img/articles/articles-1.png'; // Возвращает базовое изображение
}

const articlesDebugger = ref('');
watch(
  () => props.articles,
  (newVal) => {
    articlesDebugger.value = JSON.stringify(newVal, null, 2);
  },
  { immediate: true },
);
...
<style lang="scss">
@import './m-articles-card-comp.scss';
</style>
