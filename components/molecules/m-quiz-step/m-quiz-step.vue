<template>
  <div class="m-quiz-step">
    ...
    <div class="m-quiz-step__box">
      <label v-for="answer in answers" :key="answer.id" :for="answer.id" class="m-quiz-step__item">
        <input
          :id="answer.id"
          type="radio"
          :name="question"
          :value="answer.text"
          :test-id="`input-${sectionId}-step-answer`"
          @change="handleAnswerSelection(answer.text)"
        />
        <span class="f-text-l f-font-700">{{ answer.text }}</span>
      </label>
    </div>
    <div
      v-if="width < 576"
      class="m-quiz-step__back violet-80 f-text-xl"
      :test-id="`btn-${sectionId}-step-back-mobile`"
      @click="back"
    >
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path
...
const handleAnswerSelection = (selectedAnswer) => {
  emit('change-step', selectedAnswer);
};
