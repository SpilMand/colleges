      ...
      <form class="s-form" @submit.prevent="onSubmit">
        <div class="s-form__wrapper">
          <div class="s-form__content">
            <h2 class="s-form__title f-text-xxxl">Регистрация</h2>
            <div class="s-form__subtitle f-text-m">
              Уже есть личный кабинет?
              <span class="f-link cursor-pointer" @click="emit('openAuth')"> Войти </span>
            </div>
...
const emit = defineEmits(['openAuth', 'close', 'openConfirm', 'mailTransfer']);
...
const { meta: fnValid, handleChange: fnBlur } = useField('firstName');

const [lastName, lastNameAttrs] = defineField('lastName');
const { meta: lnValid, handleChange: lnBlur } = useField('lastName');

const [email, emailAttrs] = defineField('email');
const { meta: eValid, handleChange: eBlur } = useField('email');

const [phone, phoneAttrs] = defineField('phone');
const { meta: fValid, handleChange: fBlur } = useField('phone');
