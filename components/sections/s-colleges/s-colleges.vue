  ...
  <section class="s-colleges section-m swiper-default f-fade">
    <div class="container">
      ...
      <div class="s-colleges__box">
        <div class="s-colleges__wrapper right-border-carousel">
          <swiper v-bind="swiperConfig" class="mySwiper">
            <swiper-slide v-for="(college, index) in colleges" :key="index">
              <m-colleges-card :colleges="college" />
            </swiper-slide>
            <swiper-slide>
              <m-colleges-card :last="true" link="/colleges" />
            </swiper-slide>
          </swiper>
        </div>
      ...
      <div class="f-btn-college">
        <NuxtLink :to="`/colleges` + (selectedCityOptionSlug ? '/' + selectedCityOptionSlug : '')">
          <a-button label="Все колледжи" color="violet-5" size="medium" textSize="f-text-s" />
        </NuxtLink>
      </div>
    </div>
  </section>
...
async function fetchData() {
  try {
    ...
    if (colleges.value && colleges.value.data) {
      colleges.value = colleges.value.data.map(({ id, attributes }) => ({
        id,
        name: attributes?.name,
        logo: attributes?.calc_data?.logo_url,
      }));
    }
  } catch (error) {
