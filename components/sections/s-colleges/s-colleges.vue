<template>
  <section class="s-colleges section-m swiper-default f-fade">
    <div class="container">
      <h2 class="s-colleges__title" :class="'title-' + titleSize">{{ title }}</h2>
      <div class="s-colleges__box">
        <div class="s-colleges__wrapper right-border-carousel">
          <swiper v-bind="swiperConfig" class="mySwiper">
            <swiper-slide v-for="(college, index) in colleges.data" :key="index">
              <m-colleges-card
                :test-id="`link-m-colleges-card-${college?.id}`"
                :options="college"
                :included="colleges.included"
              />
            </swiper-slide>
            <swiper-slide>
              <m-colleges-card :test-id="`link-m-colleges-card-${college?.id}`" :last="true" link="/colleges" />
            </swiper-slide>
          </swiper>
        </div>
        <div class="s-colleges-swiper-pagination s-swiper-pagination"></div>
        <div class="s-colleges-swiper-navigation s-swiper-navigation">
          <div ref="sliderPrev" class="swiper-button-prev" test-id="btn-slider-s-colleges-prev">
          ...
          <div ref="sliderNext" class="swiper-button-next" test-id="btn-slider-s-colleges-next">
      ...
      <div class="f-btn-college">
        <a-button
          ...
          size="medium"
          textSize="f-text-s"
          test-id="link-s-colleges-all"
          @click="navigateTo(`/colleges` + (selectedCityOptionSlug ? '/' + selectedCityOptionSlug : ''))"
        />
      </div>
    </div>
  </section>
...
defineProps({
  title: { type: String, default: 'Лучшие колледжи' },
  titleSize: { type: String, default: 'h2' },
});
...
const swiperConfig = reactive({
  ...
  pagination: {
    el: '.s-colleges-swiper-pagination',
    clickable: true,
  },
...
function navigateToColleges() {
  const url = '/colleges';
  // Переход на страницу колледжей
  window.location.href = url;
}
</script>
<style lang="scss">
@import './s-colleges.scss';
</style>
