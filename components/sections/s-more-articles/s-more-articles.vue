<template>
  <section id="" class="s-more-articles section-m swiper-default">
    <h2 class="s-more-articles__title title-h2">{{ title }}</h2>
    <p class="s-more-articles__subtitle subtitle f-font-700">Мы сделали подборки статей по самым популярным темам!</p>
    <p class="s-more-articles__text f-text-l f-font-700">{{ text }}</p>
    <div class="s-more-articles__box">
      ...
      <div class="s-more-articles-swiper-navigation s-swiper-navigation">
...
defineProps({
  ...
  title: { type: String, default: 'Подборки статей' },
  text: { type: String, default: 'Выбирай свою любимую тему и смотри статьи по ней в подборке.' },
});
...
const swiperConfig = reactive({
  ...
  pagination: {
    el: '.s-more-articles-swiper-pagination',
    clickable: true,
  },
