<template>
  <section id="" class="s-other-branches section-m swiper-default">
    <h3 class="s-other-branches__title title-h3">Другие филиалы колледжа</h3>
    <div class="s-other-branches__box">
      <swiper v-bind="swiperConfig" class="mySwiper">
        <swiper-slide v-for="(branch, index) in faculties" :key="index">
          <div class="s-other-branches__slide">
            <img src="/icons/map-marker.svg" alt="" />
            <span>{{ branch.attributes.address }}</span>
          </div>
        </swiper-slide>
      </swiper>

      <div class="s-other-branches-swiper-pagination s-swiper-pagination"></div>
      <div class="s-other-branches-swiper-navigation s-swiper-navigation">
...
const swiperConfig = reactive({
  ...
  pagination: {
    el: '.s-other-branches-swiper-pagination',
    clickable: true,
  },
