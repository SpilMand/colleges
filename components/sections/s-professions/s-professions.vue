<template>
  <section class="s-professions section-l swiper-default f-fade">
    <div class="container">
...
const props = defineProps({
  ...
  title: { type: String, default: 'Востребованные профессии' },
...
const swiperConfig = reactive({
  ...
  pagination: {
    el: '.s-professions-swiper-pagination',
    clickable: true,
  },
...
async function fetchData() {
  if (props.apiProfessions && props.apiProfessions.data) {
    professions.value = props.apiProfessions.data.map(({ id, attributes }) => ({
      id,
      name: attributes?.name,
      colleges: attributes?.calc_data?.count_colleges,
      specialties: attributes?.calc_data?.count_specialties,
      professions: attributes?.calc_data?.count_professions,
    }));
  }
}
