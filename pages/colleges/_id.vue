...
const breadcrumbs = [{ label: 'Колледжи' }];
const curPage = ref(0);
const countPage = ref();
...
const pageTitle = ref('Колледжи и техникумы');
...
const filterSeo = async (data) => {
  ...
  switch (true) {
    case data.type === 'cities':
      ...
      break;
    case data.type === 'forms':
      await getForm(data.id).then(() => {
        // filtersRoutes.value[data.type] = response.data.attributes.slug;
        filtersRoutes.value[data.type] = 'forms';
      });
      break;
    case data.type === 'directions':
      await getDirection(data.id).then(() => {
        // filtersRoutes.value[data.type] = response.data.attributes.slug;
        filtersRoutes.value[data.type] = 'direction';
      });
      break;
    default:
...
const updateCurPage = async (num) => {
  ...
  colleges.value = await getColleges({
    ...
    'include': 'city,monitorings,media',
    'filter[hasSpecialties]': true,
